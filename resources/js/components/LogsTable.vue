...
const rows = ref([]), ads = ref([]), adId = ref(null), from = ref(''), to = ref(''),
    pagination = ref({page: 1, next: null, prev: null});
onMounted(async () => {
    ads.value = await (await fetch('/api/ads')).json();
    await load();
});

function url(pageToken = null) {
    const p = new URLSearchParams();
    if (adId.value) p.set('ad_id', adId.value);
...
async function load(pageToken = null) {
    const d = await (await fetch(url(pageToken))).json();
    rows.value = d.data;
    pagination.value = {
        page: d.current_page,
        next: d.next_page_url ? d.current_page + 1 : null,
        prev: d.prev_page_url ? d.current_page - 1 : null
    };
}

function move(dir) {
    const t = dir > 0 ? pagination.value.next : pagination.value.prev;
    if (t) load(t);
}
</script>
